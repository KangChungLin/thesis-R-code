MND_RH=MND_RH[,-1]
temp.lm1=lm(Jan.Jul~IMF1.5,MND_RH, na.action = na.exclude)
summary(temp.lm1)
full = formula(lm(Jan.Jul~.,MND_RH, na.action = na.exclude))

step(temp.lm1,direction='both',scope=full)
lm.fit=lm(Jan.Jul~IMF1.5+IMF.7,MND_RH)
#lm.fit=lm(Jan.Jul~IMF1.5,MND_RH)
summary(lm.fit)
...
#lm1.fit=lm(Jan.Jul~IMF1.5,t1)
#lm2.fit=lm(Jan.Jul~IMF1.5,t2)
lm1.fit=lm(Jan.Jul~IMF1.5,t1)
lm2.fit=lm(Jan.Jul~IMF1.5,t2)
summary(lm1.fit)
summary(lm2.fit)
...
plot(1949:2006,MND_RH$Jan.Jul,type = 'l',lwd=2,col='blue',las=1,xlab = "Year",
     ylab='Jan-Jul mean temperature')
lines(1949:2006,predict(lm.fit),lwd=2,col='red')
